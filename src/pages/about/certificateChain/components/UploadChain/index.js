...
function UploadChain(props) {
  const { visible, onCancel, dispatch, Union, User, addLoading = false } = props;
  ...
  return (
    <Modal {...drawerProps}>
      <Form layout="vertical" form={form}>
        ...
        >
          <MonacoEditor width="100%" height="200" language="json" theme="vs-light" value={jsonContent} options={editorOptions} onChange={onChange} editorDidMount={editorDidMount} />
        </Form.Item>
      </Form>
    </Modal>
  );
...
export default connect(({ Union, User, CertificateChain, loading }) => ({
  ...
  User,
  CertificateChain,
  addLoading: loading.effects['CertificateChain/uploadChain'],
}))(UploadChain);
