  ...
  render() {
    ...
    return (
      <div className="page-wrapper">
              ...
              <Row gutter={[16, 24]}>
                {repositoryList.map((item) => (
                  <Col key={item._id} span={8}>
                    <RepositoryCard
                      record={item}
                      deployContract={() => this.onClickDeploy(item)}
                      viewDetail={() => this.viewDetail(item)}
                    />
                  </Col>
                ))}
              </Row>
        ...
        {deployModalVisible && (
          <DeployContract visible={deployModalVisible} record={curRepository} onCancel={this.onCloseDeployModal} />
        )}
      </div>
    );
  }
