...
function EditStrategy(props) {
  const { visible, editParams = {}, onCancel, getPageListOfRoleData, operateType, dispatch, addLoading = false } = props;
  ...
  return (
    <Modal {...drawerProps}>
      <Form {...formItemLayout} form={form}>
        <Item label='策略名称' name='strategyName' initialValue={editParams.strategyName || ''} rules={[
          {
            required: true,
            message: '请输入隐私保护策略名称',
          },
        ...
        <Item label='状态' name='strategyStatus' initialValue={editParams.strategyStatus} rules={[
          {
            required: true,
            message: '请选择隐私保护策略状态',
          },
        ...
        <Item label='策略描述' name='strategyDesc' initialValue={editParams.strategyDesc || ''} rules={[
          {
            required: true,
            message: '请填写策略描述',
          },
