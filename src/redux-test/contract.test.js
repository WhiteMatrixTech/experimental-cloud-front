...
describe('contract->test',()=>{
        ...
        describe('contractModel->effects',()=>{
            ...
            test('getPageListOfRoleData->test',() => {
                const saga = effects.getPageListOfRoleData
                const actionCreator = {
                type:'common',
                payload:{
                  strategyList:[]
                ...
                const generator = saga(actionCreator,{call,put})
                let next = generator.next()
                expect(next.value).toEqual(call(API.getPageListOfRoleData,actionCreator.payload))
                generator.next({
                  statusCode : 'ok',
                  result: 9
                  })
            ...
            test('getRoleDateTotalDocs->test',() => {
                const saga = effects.getRoleDateTotalDocs
                const actionCreator = {
                type:'common',
                payload:{
                  strategyTotal:0
                ...
                const generator = saga(actionCreator,{call,put})
                let next = generator.next()
                expect(next.value).toEqual(call(API.getRoleDateTotalDocs,actionCreator.payload))
                generator.next({
                  statusCode : 'ok',
                  result: 9
                  })
