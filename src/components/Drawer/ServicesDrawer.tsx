...
const ServicesDrawer: React.FC<ServicesDrawerProps> = (props) => {
  ...
  const getNetworkMenu = (menuList: IMenuPathProps[]) => {
    return menuList.map((menu) => (
      <Menu.Item key={menu.finalPath} title={menu.finalPathName} onClick={() => onClickMenuPath(menu.finalPath)}>
        {/* {menu.allPath.join(' > ')} */}
        {menu.finalPathName}
      </Menu.Item>
    ));
    ...
    <Drawer
      ...
      <div className={styles['service-wrapper']}>
        <div className={styles['left-service-wrapper']}>
          ...
          <div title={currentServices.serviceName} className={styles['current-service']}>
