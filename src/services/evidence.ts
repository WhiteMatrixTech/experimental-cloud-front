...
export async function getEvidenceDataByHash(params: { networkName: string; evidenceHash: string }) {
  return request(`/network/${params.networkName}/evidences/query/${params.evidenceHash}`);
}

// 点击查看详情
export async function getEvidenceDetail(params: BasicApiParams & { evidenceHash: string }) {
  return request(`/network/${params.networkName}/evidences/channel/${params.channelId}/${params.evidenceHash}`, {
    method: 'GET',
    body: params,
  });
}
