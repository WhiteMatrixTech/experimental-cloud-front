...
// 根据存证哈希查询存证上链数据
export async function getEvidenceDataByHash(params: { networkName: string; evidenceHash: string, channelName: string; }) {
  return request(`/network/${params.networkName}/evidences/${params.channelName}/${params.evidenceHash}`);
}

// 点击查看详情
export async function getEvidenceDetail(params: BasicApiParams & { evidenceHash: string }) {
  return request(`/network/${params.networkName}/evidences/${params.channelId}/${params.evidenceHash}`, {
    method: 'GET',
    body: params,
  });
}
